<script lang="ts">
  ...
  let hue = chroma.hex($config.colorNickname).hsv()[0];
  let saturation = chroma.hex($config.colorNickname).hsv()[1];
  let value = chroma.hex($config.colorNickname).hsv()[2];
...
<TextField title={$LL.followAlerts.controls.colorNickname()} class="mb-4">
  <div class="w-2/3 xl:w-1/2">
    ...
    <Input value={$config.colorNickname} on:input={handleInput} />
  </div>
</TextField>
