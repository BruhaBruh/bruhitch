<script lang="ts">
  ...
  let hue = chroma.hex($config.textColor).hsv()[0];
  let saturation = chroma.hex($config.textColor).hsv()[1];
  let value = chroma.hex($config.textColor).hsv()[2];
  ...
  config.subscribe((v) => {
    const color = chroma.hex(v.textColor).hsv();
    saturation = color[1];
    value = color[2];
  });
</script>

<TextField title={$LL.donationAlerts.controls.textColor()} class="mb-4">
  <div class="w-2/3 xl:w-1/2">
    ...
    <Input value={$config.textColor} on:input={handleInput} />
  </div>
</TextField>
