<script lang="ts">
  ...
  let hue = chroma.hex($config.soundColor).hsv()[0];
  let saturation = chroma.hex($config.soundColor).hsv()[1];
  let value = chroma.hex($config.soundColor).hsv()[2];
...
<TextField title={$LL.donationAlerts.controls.soundColor()} class="mb-4">
  <div class="w-2/3 xl:w-1/2">
    ...
    <Input value={$config.soundColor} on:input={handleInput} />
  </div>
</TextField>
