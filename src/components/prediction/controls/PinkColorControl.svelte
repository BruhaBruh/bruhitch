<script lang="ts">
  ...
  let hue = chroma.hex($config.pinkColor).hsv()[0];
  let saturation = chroma.hex($config.pinkColor).hsv()[1];
  let value = chroma.hex($config.pinkColor).hsv()[2];
  ...
  config.subscribe((v) => {
    const color = chroma.hex(v.pinkColor).hsv();
    hue = color[0];
    saturation = color[1];
    value = color[2];
  });
...
<TextField title={$LL.predictionWidget.controls.pinkColor()} class="mb-4">
  <div class="w-2/3 xl:w-1/2">
    ...
    <Input value={$config.pinkColor} on:input={handleInput} />
  </div>
</TextField>
