...
<div {...$$restProps}>
  <TextField title={$LL.subscribeAlerts.controls.animation()} class="mb-4">
  ...
  {#if animation === Animation.Scale}
    <TextField title={$LL.subscribeAlerts.controls.animationParams.name()}>
      <div class={'flex space-x-2'}>
        <Input
          class="flex-1"
          bind:value={start}
        ...
        <Input
          class="flex-1"
          bind:value={opacity}
          placeholder={$LL.subscribeAlerts.controls.animationParams.opacity()}
        />
      </div>
    </TextField>
  {/if}
